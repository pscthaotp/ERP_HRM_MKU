...
{
    public static class Prosess_QuyetDinhThongBaoChamDutThuViec
    {
        public static void ShowMailMerge(IObjectSpace obs, List<QuyetDinhThongBaoChamDutThuViec> qdList)
        {
            var non_QuyetDinhHTList = new List<Non_QuyetDinhThongBaoChamDutThuViec>();
            var non_QuyetDinhHDQTList = new List<Non_QuyetDinhThongBaoChamDutThuViec>();
            Non_QuyetDinhThongBaoChamDutThuViec qd;
            foreach (QuyetDinhThongBaoChamDutThuViec quyetDinh in qdList)
            {
                qd = new Non_QuyetDinhThongBaoChamDutThuViec();
            ...
            {
                MailMergeTemplate merge = Common.GetTemplate(obs, "QuyetDinhThongBaoChamDutThuViec.rtf", qdList[0].CongTy.Oid);
                if (merge != null)
                    Prosess_Show.ShowEditor<Non_QuyetDinhThongBaoChamDutThuViec>(non_QuyetDinhHTList, obs, merge);
                else
            ...
            if (non_QuyetDinhHDQTList.Count > 0)
            {
                MailMergeTemplate merge = Common.GetTemplate(obs, "QuyetDinhThongBaoChamDutThuViec.rtf", qdList[0].CongTy.Oid);
                if (merge != null)
                    Prosess_Show.ShowEditor<Non_QuyetDinhThongBaoChamDutThuViec>(non_QuyetDinhHDQTList, obs, merge);
                else
                    DialogUtil.ShowError("Không tìm thấy mẫu Quyết định trong hệ thống.");
            }
        }
