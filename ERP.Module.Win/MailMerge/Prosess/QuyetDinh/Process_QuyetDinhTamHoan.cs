...
{
    public static class Prosess_QuyetDinhTamHoan
    {
        public static void ShowMailMerge(IObjectSpace obs, List<QuyetDinhTamHoan> qdList)
        {
            var non_QuyetDinhHTList = new List<Non_QuyetDinhTamHoan>();
            var non_QuyetDinhHDQTList = new List<Non_QuyetDinhTamHoan>();
            Non_QuyetDinhTamHoan qd;
            foreach (QuyetDinhTamHoan quyetDinh in qdList)
            {
                qd = new Non_QuyetDinhTamHoan();
             ...
             //   qd.LoaiHopDong = quyetDinh.ThongTinNhanVien.LoaiHopDong != null ? quyetDinh.ThongTinNhanVien.LoaiHopDong.TenLoaiHopDong : "";
                qd.HopDongLaoDongSo = quyetDinh.HopDong != null ? quyetDinh.HopDong.SoHopDong.ToString() : ".....";
                qd.NgayKyHopDong = quyetDinh.HopDong != null ? quyetDinh.HopDong.NgayKy.ToString("dd/MM/yyyy") : ".....";
                ...
                //
                qd.NgayHetTamHoan = quyetDinh.NgayHetTamHoan != DateTime.MinValue ? quyetDinh.NgayHetTamHoan.ToString("dd/MM/yyyy") : "";
                qd.SoThang = quyetDinh.SoThangTamHoan.ToString();
                qd.SoNam = Common.GetYearNumber(quyetDinh.NgayHieuLuc, quyetDinh.NgayHetTamHoan).ToString();
                //
            ...
            {
                MailMergeTemplate merge = Common.GetTemplate(obs, "QuyetDinhTamHoan.rtf", qdList[0].CongTy.Oid);
                if (merge != null)
                    Prosess_Show.ShowEditor<Non_QuyetDinhTamHoan>(non_QuyetDinhHTList, obs, merge);
                else
            ...
            if (non_QuyetDinhHDQTList.Count > 0)
            {
                MailMergeTemplate merge = Common.GetTemplate(obs, "QuyetDinhTamHoan.rtf", qdList[0].CongTy.Oid);
                if (merge != null)
                    Prosess_Show.ShowEditor<Non_QuyetDinhTamHoan>(non_QuyetDinhHDQTList, obs, merge);
                else
                    DialogUtil.ShowError("Không tìm thấy mẫu Quyết định trong hệ thống.");
            }
        }
