...
{
    public static class Prosess_QuyetDinhBietPhai
    {
        public static void ShowMailMerge(IObjectSpace obs, List<QuyetDinhBietPhai> qdList)
        {
            var non_QuyetDinhHTList = new List<Non_QuyetDinhBietPhai>();
            var non_QuyetDinhHDQTList = new List<Non_QuyetDinhBietPhai>();
            Non_QuyetDinhBietPhai qd;
            foreach (QuyetDinhBietPhai quyetDinh in qdList)
            {
                qd = new Non_QuyetDinhBietPhai();
            ...
            {
                MailMergeTemplate merge = Common.GetTemplate(obs, "QuyetDinhBietPhai.rtf", qdList[0].CongTy.Oid);
                if (merge != null)
                    Prosess_Show.ShowEditor<Non_QuyetDinhBietPhai>(non_QuyetDinhHTList, obs, merge);
                else
            ...
            if (non_QuyetDinhHDQTList.Count > 0)
            {
                MailMergeTemplate merge = Common.GetTemplate(obs, "QuyetDinhBietPhai.rtf", qdList[0].CongTy.Oid);
                if (merge != null)
                    Prosess_Show.ShowEditor<Non_QuyetDinhBietPhai>(non_QuyetDinhHDQTList, obs, merge);
                else
                    DialogUtil.ShowError("Không tìm thấy mẫu Quyết định trong hệ thống.");
            }
        }
