...
{
    public static class Prosess_QuyetDinhThoiViec
    {
        public static void ShowMailMerge(IObjectSpace obs, List<QuyetDinhThoiViec> qdList)
        {
            var non_QuyetDinhList = new List<Non_QuyetDinhThoiViec>();
            ...
            foreach (QuyetDinhThoiViec quyetDinh in qdList)
            {
                ...
                qd.ThoiHanBanGiaoCongViec = quyetDinh.ThoiHanBanGiao != DateTime.MinValue ? quyetDinh.ThoiHanBanGiao.ToString("d") : "";
            ...
            {
                MailMergeTemplate merge = Common.GetTemplateWithValidDate(obs, "QuyetDinhThoiViec.rtf", qdList[0].CongTy.Oid, qdList[0].NgayHieuLuc);
                if (merge != null)
                    Prosess_Show.ShowEditor<Non_QuyetDinhThoiViec>(non_QuyetDinhList, obs, merge);
                else
