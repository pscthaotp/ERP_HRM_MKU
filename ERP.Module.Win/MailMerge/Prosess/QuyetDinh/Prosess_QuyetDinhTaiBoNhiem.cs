...
{
    public static class Prosess_QuyetDinhTaiBoNhiem
    {
        public static void ShowMailMerge(IObjectSpace obs, List<QuyetDinhTaiBoNhiem> qdList)
        {
            var non_QuyetDinhHTList = new List<Non_QuyetDinhTaiBoNhiem>();
            var non_QuyetDinhHDQTList = new List<Non_QuyetDinhTaiBoNhiem>();
            Non_QuyetDinhTaiBoNhiem qd;
            foreach (QuyetDinhTaiBoNhiem quyetDinh in qdList)
            {
                qd = new Non_QuyetDinhTaiBoNhiem();
                ...
                qd.ChucDanhMoi = quyetDinh.ChucDanhCu != null ? quyetDinh.ChucDanhCu.TenChucDanh : "";
                qd.PhuCapKiemNhiemMoi = quyetDinh.PhuCapKiemNhiemCu.ToString();
            ...
            {
                MailMergeTemplate merge = Common.GetTemplateWithValidDate(obs, "QuyetDinhTaiBoNhiemHT.rtf", qdList[0].CongTy.Oid, qdList[0].NgayHieuLuc);
                if (merge != null)
                    Prosess_Show.ShowEditor<Non_QuyetDinhTaiBoNhiem>(non_QuyetDinhHTList, obs, merge);
                else
            ...
            {
                MailMergeTemplate merge = Common.GetTemplateWithValidDate(obs, "QuyetDinhTaiBoNhiemHDQT.rtf", qdList[0].CongTy.Oid, qdList[0].NgayHieuLuc);
                if (merge != null)
                    Prosess_Show.ShowEditor<Non_QuyetDinhTaiBoNhiem>(non_QuyetDinhHDQTList, obs, merge);
                else
                    DialogUtil.ShowError("Không tìm thấy mẫu Quyết định trong hệ thống.");
            }
        }
