...
{
    public class Imp_QuyetDinhKyLuat
    {
        #region 1. Quyết định kỷ luật
        public static void ImportQuyetDinhKyLuat(IObjectSpace obs, QuyetDinh_ChonNguoiKy obj)
        {
            //
            int sucessNumber = 0;
            int erorrNumber = 0;
                ...
                if (open.ShowDialog() == DialogResult.OK)
                {
                    using (DialogUtil.AutoWait())
                    {
                        using (DataTable dt = DataProvider.GetDataTableFromExcel(open.FileName, "[Sheet1$A1:K]", obj.LoaiOffice))
                        {
                            ...
                            int idx_TuNgay = 4;
                            int idx_DenNgay = 5;
                            int idx_SoQuyetDinh = 6;
                            int idx_NgayQuyetDinh = 7;
                            int idx_LyDo = 8;
                            #endregion
                                ...
                                foreach (DataRow dr in dt.Rows)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(txt_MaQuanLy))
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                quyetDinh = uow.FindObject<QuyetDinhKyLuat>(CriteriaOperator.Parse("CongTy = ? and SoQuyetDinh like ?", obj.CongTy.Oid, txt_SoQuyetDinh));
                                                if (quyetDinh == null)
                                                {
                                                    quyetDinh = new QuyetDinhKyLuat(uow);
                                                ...
                                                #region 1. Hình thức kỷ luật
                                                if (!string.IsNullOrEmpty(txt_HinhThuc))
                                                {
                                                    HinhThucKyLuat hinhThuc = uow.FindObject<HinhThucKyLuat>(CriteriaOperator.Parse("MaQuanLy=?", txt_HinhThuc));
                                                    if (hinhThuc != null)
                                                    {
                                                        quyetDinh.HinhThucKyLuat = hinhThuc;
                                                    }
                                                    else
                                                    {
                                                        detailLog.AppendLine("+ Mã quản lý Hình thức kỷ luật không hợp lệ.");
                                                    }
                                                }
                                                else
                                                {
                                                    detailLog.AppendLine("+ Thiếu thông tin Hình thức kỷ luật.");
                                                }
                                                #endregion
                                                ...
                                                {
                                                    quyetDinh.TuNgay = Convert.ToDateTime(txt_TuNgay);
                                                    quyetDinh.NgayHieuLuc = quyetDinh.TuNgay;
                                                }
                                                else
                                                {
                                                ...
                                                {
                                                    quyetDinh.DenNgay = Convert.ToDateTime(txt_DenNgay);
                                                }
                                                else
                                                {
                                                    detailLog.AppendLine("+ Thiếu thông tin Đến ngày.");
                                                }
                                                #endregion
