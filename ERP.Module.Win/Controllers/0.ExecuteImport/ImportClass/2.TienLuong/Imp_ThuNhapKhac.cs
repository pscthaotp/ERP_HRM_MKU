...
{
    public class Imp_ThuNhapKhac
    {
        #region 1.Thu nhập khác
        public static void ImportThuNhapKhac(IObjectSpace obs, BangThuNhapKhac obj, LoaiOfficeEnum loaiOffice)
        {
            //
            int sucessNumber = 0;
            int erorrNumber = 0;
                                ...
                                foreach (DataRow dr in dt.Rows)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(txt_MaQuanLy))
                                    {
                                        ...
                                        if (nhanVien_qd == null)
                                        {
                                            ...
                                            mainLog.AppendLine(string.Format("- Mã quản lý :{0} của nhân viên : {1} không tồn tại trong hệ thống của {2}.", txt_MaQuanLy, txt_HoTen, obj.CongTy.TenBoPhan));
                                            //
                                            sucessImport = false;
                                        }
                                        else
                                        {
                                            ...
                                            if (nhanVien != null)
                                            {
                                                ChiTietThuNhapKhac chiTietThuNhapKhac = uow.FindObject<ChiTietThuNhapKhac>(CriteriaOperator.Parse("BangThuNhapKhac=? and ThongTinNhanVien=?", obj.Oid, nhanVien.Oid));
                                                ...
                                                if (!string.IsNullOrEmpty(txt_SoTien))
                                                {
                                                    try
                                                    {
                                                        decimal soTien = Convert.ToDecimal(txt_SoTien);
                                                        //
                                                        chiTietThuNhapKhac.SoTien = soTien;
                                                    }
                                                    catch
                                                    {
                                                        detailLog.AppendLine("Số tiền không hợp lệ: " + txt_SoTien);
                                                    }
                                                }
