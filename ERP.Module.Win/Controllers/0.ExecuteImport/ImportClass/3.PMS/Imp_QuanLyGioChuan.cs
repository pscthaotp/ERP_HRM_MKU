...
{  
    public class Imp_QuanLyGioChuan
    {
        ...
        public static void XuLy(IObjectSpace obs, Guid OidQuanLy)
        {
            ...
            using (OpenFileDialog open = new OpenFileDialog())
            {
                ...
                if (open.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        using (DataTable dt = DataProvider.GetDataTableFromExcel(open.FileName, "[Sheet1$A2:E]", LoaiOfficeEnum.Office2003))
                        {
                            /////////////////////////////KHỞI TẠO CÁC IDX/////////////////////////////////////////////////////
                            ...
                            const int idx_DinhMucGioDay = 2;
                            const int idx_DinhMucNCKH = 3;
                            const int idx_GhiChu = 4;
                            string sql = "";
                            #endregion
                            ...
                            using (UnitOfWork uow = new UnitOfWork(((XPObjectSpace)obs).Session.DataLayer))
                            {
                                ...
                                foreach (DataRow dr in dt.Rows)
                                {
                                    ...
                                    string txt_DinhMucGioDay = dr[idx_DinhMucGioDay].ToString().Trim();
                                    string txt_DinhMucNCKH = dr[idx_DinhMucNCKH].ToString().Trim();
                                    ...
                                    if (!string.IsNullOrEmpty(txt_MaGV))
                                    {
                                        ...
                                        else
                                        {
                                            sql += " Union All select N'" + nhanVien.Oid + "' as NhanVien"
                                                          + ", '" + txt_DinhMucGioDay.Replace(",", ".") + "' as DinhMucGioDay"
                                                          + ", '" + txt_DinhMucNCKH.Replace(",", ".") + "' as DinhMucNCKH"
                                ...
                                if (sucessImport)
                                {
                                    ...
                                    DataProvider.ExecuteNonQuery("spd_PMS_Import_QuanLyGioChuan", CommandType.StoredProcedure, pImport);
                                    //Lưu
                                    //uow.CommitChanges();
                                    //
                                    sucessNumber++;
